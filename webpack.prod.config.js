...
const distDir = `${path.resolve('dist')}/build`
// 用于复制不参与编译的文件
const copyMap = [
    // 插件配置文件
    {
...
module.exports = {
    ...
    entry: {
        background: "./src/background.ts", popup: "./src/popup.ts", content: './src/content.ts'
    },
    ...
    // 指定打包文件所在目录
    output: {
        path: path.resolve(__dirname, `dist/build`), filename: "js/[name].js", // 编译打包后的js文件名称
    },
    // 用来指定那些模块可以用来备注引入
    resolve: {
        extensions: ['.ts', ',js']
